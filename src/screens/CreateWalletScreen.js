...
class CreateWalletScreen extends Component {
	...
	_createWallet = () => {
		let wallet = NULS.Account.create(this.state.password);
	...
	_renderContent = () => {
		...
		switch(this.state.walletType) {
		  case 'new-wallet':
		    ...
		    break;
		  case 'private-key':
				contents.push(<InputField key={0} label="Private Key" onChangeText={(value)=>{this.setState({importedPrivateKey:value})}} />);
		    contents.push(<PrimaryButton key={4} title="Create Wallet" onPress={() => NULS.Account.import(null, this.state.importedPrivateKey)}/>);
		    break;
		  case 'encrypted-key':
				contents.push(<InputField key={0} label="Private Key" onChangeText={(value)=>{this.setState({importedPrivateKey:value})}}/>);
				contents.push(<InputField key={1} label="Passphrase" onChangeText={(value)=>{this.setState({encryptionPhrase:value})}} carretHidden/>);
		    contents.push(<PrimaryButton key={4} title="Create Wallet" onPress={() => NULS.Account.import(this.state.encryptionPhrase, this.state.importedPrivateKey)}/>);
		    break;
		  default:
		   	break;
		}
